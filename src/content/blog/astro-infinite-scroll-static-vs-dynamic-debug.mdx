...
import { Alert, Card, Mermaid } from '../../components/MDXComponents.jsx';
...
## 第八章：深入理解 - 前端 API 的互動機制

在解決了技術問題後，讓我們深入理解這個架構是如何運作的。

### 傳統架構 vs Astro 混合架構

#### 傳統前後端分離
```
[React 前端] → HTTP 請求 → [Node.js 後端] → 資料庫 → JSON 回應 → [前端渲染]
   (3000 port)              (8080 port)
```
...
```typescript
// 用戶訪問 /blog
// Astro 返回預渲染的 HTML（包含前 6 篇文章）
// HTML 中包含 React 組件 <InfiniteScrollBlog />
```
...
const loadMorePosts = async () => {
  // 1. 前端發起請求
  const response = await fetch(`/api/posts.json?page=${currentPage}&limit=6`);
...
| 路由類型 | 渲染方式 | 使用場景 |
|---------|---------|---------|
| `/blog/[slug]` | 預渲染（構建時生成） | SEO 重要的文章頁 |
| `/api/posts.json` | 動態執行（請求時運行） | 分頁數據載入 |

#### 3. Netlify 部署的角色分工
```
Netlify CDN
├── 靜態文件（HTML、CSS、JS）→ 直接提供
└── API 路由 → Netlify Functions 執行

當請求 /api/posts.json?page=2 時：
1. Netlify 識別這是 API 路由
2. 啟動 Serverless Function
3. 執行你的 TypeScript 代碼
4. 返回 JSON 結果
```
...
<Alert type="info">
**購物網站的商品列表**
- 初始載入：20 個熱門商品（靜態 HTML，SEO 友好）
- 滾動載入：API 獲取更多商品（動態數據）
- 優勢：快速首屏 + 無限商品 + 良好 SEO

**你的部落格**
- 初始載入：6 篇最新文章（靜態 HTML，Google 可索引）
- 滾動載入：API 獲取歷史文章（動態請求）
- 優勢：快速載入 + 無限滾動 + SEO 優化
</Alert>
...
sequenceDiagram
    ...
    participant R as React 組件
    participant A as Astro API
    participant C as Content Collection
    
    U->>R: 滾動到底部
    R->>R: Intersection Observer 觸發
    R->>A: fetch('/api/posts.json?page=2')
    A->>C: getCollection('blog')
    C->>A: 返回所有文章
    A->>A: 計算第 2 頁（7-12 篇）
    A->>R: JSON: {posts: [...], hasMore: false}
    R->>R: setPosts([...old, ...new])
    R->>U: 渲染新文章到頁面
`} />
...
**技術堆疊**：
- Astro v5 Static 模式（含 hybrid 功能）
