...
<Alert type="error">
驚訝發現：Astro 5.x 版本竟然不接受 `hybrid` 模式！
</Alert>

## 第五章：最終解決方案 - Server 模式 + 選擇性預渲染
...
Server 模式帶來新問題：文章頁面變成空白！

```bash
# 錯誤日誌
[ERROR] Cannot read properties of undefined (reading 'data')
  Stack trace:
    at Object.default (/src/pages/blog/[page].astro:42:15)
```

原因：`getStaticPaths()` 在 server 模式下被忽略，導致 `page` 變數 undefined。

### 步驟 4：選擇性預渲染

```typescript
// src/pages/blog/[...slug].astro
export const prerender = true;  // 文章頁面需要預渲染
export async function getStaticPaths() {
  // 現在這個函數會正常執行
}

// src/pages/blog/[page].astro
export const prerender = true;  // 分頁頁面也需要預渲染

// src/pages/api/posts.json.ts
export const prerender = false; // API 保持動態（默認就是 false）
```
...
```javascript
output: 'hybrid' // Astro 5.x 不支持！
```

- **特點**：理論上默認靜態，可選擇性設置動態
- **現實**：Astro 5.x 版本報錯，無法使用
- **替代方案**：使用 Server 模式 + 選擇性預渲染

### 4. Server + 選擇性預渲染（實際解決方案）

```javascript
output: 'server'
adapter: netlify()  // 必須有適配器
...
- **特點**：默認動態，選擇性靜態
- **優勢**：達到 Hybrid 模式相同效果
- **部署**：需要適配器支持（Netlify、Vercel 等）

<Alert type="success">
**實戰結論**：Server 模式 + 選擇性預渲染 = 實際可用的 Hybrid 模式
</Alert>
...
- ❌ 在錯誤方向上除錯（參數解析）：3 小時
- ❌ Hybrid 模式失敗並尋找替代方案：30 分鐘
- ❌ 修復 Server 模式下的文章空白問題：20 分鐘
- ✅ 最終實施 Server + 預渲染方案：15 分鐘
...
| 需要搜尋、評論 | Server + prerender | Hybrid 不可用，用此替代 |
| 社交平台、論壇 | Server | 高度動態互動 |
| Astro 5.x 動態功能 | Server + 選擇性 prerender | 唯一可行方案 |
...
*除錯時間：2025年8月14日凌晨*  
*框架版本：Astro 5.12.2*  
*預期方案：Static → Hybrid 模式*  
*實際方案：Static → Server + 選擇性預渲染*  
*關鍵發現：Astro 5.x 不支持 hybrid 模式*